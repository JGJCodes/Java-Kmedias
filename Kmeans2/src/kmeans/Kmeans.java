...
public class Kmeans {
    ...
    public int Algoritmo(char distancia){
        ...
        do{
            ...
            Main.ImprimirResultados(this, iteraciones);
            iteraciones+=1;
            centInicio = Centroides;
            Centroides = CalcularCentroides(iteraciones);
        }while(!(this.evaluarCentroides(centInicio,Centroides)));
        return iteraciones;
    }
    
    public boolean evaluarCentroides(Ejemplo cent1[], Ejemplo cent2[]){
        boolean igualdad = true;
        for (int i=0; i<cent1.length;i++) {
            if(cent1[i].getX()!=cent2[i].getX() && cent1[i].getY()!=cent2[i].getY())
                igualdad = false;
        }
        return igualdad;
    }
